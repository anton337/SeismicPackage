...
void set_data_vec_ptr ( vect * _data )
{
    data_vec = _data;
}
...
void drawStuff(void)
{
    ...
    if ( data_vec )
    {
            ...
            )
        {
            ...
            {
                r = sqrtf ( data_vec[x*num_z+z].vx*data_vec[x*num_z+z].vx + data_vec[x*num_z+z].vz*data_vec[x*num_z+z].vz );
                if ( r > 1e-5 )
                {
                    if ( r > 20 ) r = 20/r; else r = 1;
                    glVertex3f(-1+2*(float)x/num_x,0,-1+2*(float)z/num_z);
                    glVertex3f(-1+2*(float)(x+r*data_vec[x*num_z+z].vx)/num_x,0,-1+2*(float)(z+r*data_vec[x*num_z+z].vz)/num_z);
                }
            }
        }
        glEnd();
