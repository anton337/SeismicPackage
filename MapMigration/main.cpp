                                              ...
                                              )
{
    ...
    {
        // c33    vp0_sqr = rho * fact * vel[t+x*num_t] * vel[t+x*num_t];
        // c11       vpx_sqr = vp0_sqr * ( 1 + 2 * epsilon[t+x*num_t] );
        //        vpn_sqr = vp0_sqr * ( 1 + 2 * delta  [t+x*num_t] );
        // c44       vs0_sqr = rho * fact * vs [t+x*num_t] * vs [t+x*num_t];
        // c13 + c44       cross = sqrtf ( ( vp0_sqr - vs0_sqr ) * ( vpn_sqr - vs0_sqr ) );
        c33 = vp[k] * vp[k] * rho[k];
        ...
        c13 = rho[0] * sqrtf( ( vp[k]*vp[k] - vs[k]*vs[k] ) * ( vp[k]*vp[k]*(1+2*((delta)?delta[k]:0)) - vs[k]*vs[k] ) ) - c44;
        // c13 = rho[k] * sqrtf( ( vp[k]*vp[k] - 0*vs[k]*vs[k] ) * ( vp[k]*vp[k]*(1+2*((delta)?delta[k]:0)) - 0*vs[k]*vs[k] ) );
        if ( k == 0 ) std::cout << c13 << std::endl;
        ...
        *bond_t = matrix < float > ( 6 , 6 , stiffness_x );
        ...
        //     [ c15 c56 c55 ]
        //     c55 = c44 for VTI
        if(A11)A11[k] = stiffness_t[0] ; if(A12)A12[k] = stiffness_t[5] ; if(A13)A13[k] = stiffness_t[4] ;
        if(A21)A21[k] = stiffness_t[30]; if(A22)A22[k] = stiffness_t[35]; if(A23)A23[k] = stiffness_t[29];
        if(A31)A31[k] = stiffness_t[24]; if(A32)A32[k] = stiffness_t[34]; if(A33)A33[k] = stiffness_t[28];
        // Q = [ c55 c45 c35 ]
        //     [ c45 c44 c34 ]
        //     [ c35 c34 c33 ]
        //     c55 = c44 for VTI
        if(Q11)Q11[k] = stiffness_t[28]; if(Q12)Q12[k] = stiffness_t[22]; if(Q13)Q13[k] = stiffness_t[16];
        if(Q21)Q21[k] = stiffness_t[27]; if(Q22)Q22[k] = stiffness_t[21]; if(Q23)Q23[k] = stiffness_t[15];
        if(Q31)Q31[k] = stiffness_t[26]; if(Q32)Q32[k] = stiffness_t[34]; if(Q33)Q33[k] = stiffness_t[14];
        // C = [ c15 c14 c13 ]
        //     [ c56 c46 c36 ]
        //     [ c55 c45 c35 ]
        if(C11)C11[k] = stiffness_t[4] ; if(C12)C12[k] = stiffness_t[3] ; if(C13)C13[k] = stiffness_t[2] ;
        if(C21)C21[k] = stiffness_t[29]; if(C22)C22[k] = stiffness_t[23]; if(C23)C23[k] = stiffness_t[17];
        if(C31)C31[k] = stiffness_t[28]; if(C32)C32[k] = stiffness_t[22]; if(C33)C33[k] = stiffness_t[16];
        // G = [ c15 c56 c55 ]
        //     [ c14 c46 c45 ]
        //     [ c13 c36 c35 ]
        if(G11)G11[k] = stiffness_t[4] ; if(G12)G12[k] = stiffness_t[29]; if(G13)G13[k] = stiffness_t[28];
        if(G21)G21[k] = stiffness_t[3] ; if(G22)G22[k] = stiffness_t[23]; if(G23)G23[k] = stiffness_t[22];
        if(G31)G31[k] = stiffness_t[2] ; if(G32)G32[k] = stiffness_t[17]; if(G33)G33[k] = stiffness_t[16];
        if ( k == 0 )
        {
            {
                           ...
                           )
{
    ...
    if ( cx )
    {
            ...
            )
        {
            ...
            {
                {
                    //if ( t==1 && x==1 ) std::cout << C13[t+x*num_t] << std::endl;
                    //
                    // A = [ (c11)  c16   c15  ]
                    //     [  c16  (c66)  c56  ]
                    //     [  c15   c56  (c55) ]
                    ...
                    //     [ (c13) c36  c35  ]
                    //
                    cx[t+x*num_t] = 0.5 * ( ( ( (A11&&p1x)?( ( A11[t+(x+1)*num_t] + A11[t+x*num_t] ) * ( p1x[t    +(x+1)*num_t] - p1x[t    +x    *num_t] ) ): 0 )
                                  ...
                                  + 0.25 * ( ( ( (C11&&p1x)? ( C11[t  +(x+1)*num_t] * ( p1x[t+1+(x+1)*num_t] - p1x[t-1+(x+1)*num_t] ) - C11[t  +(x-1)*num_t] * ( p1x[t+1+(x-1)*num_t] - p1x[t-1+(x-1)*num_t] ) ) :0 )
                                  ...
                                  -     p2x[t+x*num_t]
                                  ;
                // cx[t+x*num_t] = vpx_sqr * ( -2*p1x[t+x*num_t] 
                //                           +    p1x[t+(x+1)*num_t]
                //                           +    p1x[t+(x-1)*num_t]
                //                           )
                //               + vs0_sqr * ( -2*p1x[t+x*num_t] 
                //                           +    p1x[t+1+x*num_t]
                //                           +    p1x[t-1+x*num_t]
                //                           )
                //               + cross   * ( -2*p1z[t+x*num_t] 
                //                           +    p1z[t+(x+1)*num_t]
                //                           +    p1z[t+(x-1)*num_t]
                //                           )
                //               + 2 * p1x[t+x*num_t]
                ...
                //               ;
                // cy[t+x*num_t] = 0;
                // cz[t+x*num_t] = vs0_sqr * ( -2*p1z[t+x*num_t] 
                //                           +    p1z[t+(x+1)*num_t]
                //                           +    p1z[t+(x-1)*num_t]
                //                           )
                //               + vp0_sqr * ( -2*p1z[t+x*num_t] 
                //                           +    p1z[t+1+x*num_t]
                //                           +    p1z[t-1+x*num_t]
                //                           )
                //               + cross   * ( -2*p1x[t+x*num_t] 
                //                           +    p1x[t+1+x*num_t]
                //                           +    p1x[t-1+x*num_t]
                //                           )
                //               + 2 * p1z[t+x*num_t]
    ...
    if ( cy )
    {
            ...
            )
        {
                ...
                )
            {
                {
                    cy[t+x*num_t] = 
                                    0.5 * ( ( ( (A21&&p1x)?( ( A21[t+(x+1)*num_t] + A21[t+x*num_t] ) * ( p1x[t    +(x+1)*num_t] - p1x[t    +x    *num_t] ) ): 0 )
                                   ...
                                   + 0.25 * ( ( ( (C21&&p1x)? ( C21[t  +(x+1)*num_t] * ( p1x[t+1+(x+1)*num_t] - p1x[t-1+(x+1)*num_t] ) - C21[t  +(x-1)*num_t] * ( p1x[t+1+(x-1)*num_t] - p1x[t-1+(x-1)*num_t] ) ) :0 ) 
    ...
    if ( cz )
    {
            ...
            )
        {
            ...
            {
                {
                    cz[t+x*num_t] = 0.5 * ( ( ( (A31&&p1x)?( ( A31[t+(x+1)*num_t] + A31[t+x*num_t] ) * ( p1x[t    +(x+1)*num_t] - p1x[t    +x    *num_t] ) ): 0 )
                                  ...
                                  + 0.25 * ( ( ( (C31&&p1x)? ( C31[t  +(x+1)*num_t] * ( p1x[t+1+(x+1)*num_t] - p1x[t-1+(x+1)*num_t] ) - C31[t  +(x-1)*num_t] * ( p1x[t+1+(x-1)*num_t] - p1x[t-1+(x-1)*num_t] ) ) :0 )
...
void propagate_anisotropic_2 ( float h 
                             , int num_x 
                             , int num_t
                             , float * p2x
                             , float * p1x
                             ...
                             )
{
        ...
        )
    {
        ...
        {
            {
                vp0_sqr = fact * vel[t+x*num_t] * vel[t+x*num_t];
                vpx_sqr = vp0_sqr * ( 1 + 2 * epsilon[t+x*num_t] );
                vpn_sqr = vp0_sqr * ( 1 + 2 * delta  [t+x*num_t] );
                vs0_sqr = fact * vs [t+x*num_t] * vs [t+x*num_t];
                cross = sqrtf ( ( vp0_sqr - vs0_sqr ) * ( vpn_sqr - vs0_sqr ) );
                cx[t+x*num_t] = vpx_sqr * ( -2*p1x[t+x*num_t] 
                                          +    p1x[t+(x+1)*num_t]
                                          +    p1x[t+(x-1)*num_t]
                                          )
...
// x2 y2
// ...
void read_horizon ( std::string file_name , std::vector < pt > & h )
{
    std::string line;
...
void write_horizon ( std::string file_name , std::vector < pt > const & h )
{
    std::string line;
    ...
    if ( myfile . is_open () )
    {
        ...
        {
            myfile << h[k].x_pos << " " << h[k].z_pos << std::endl;
        }
    }
    else
    {
...
void interpolate ( std::vector < pt > in , std::vector < pt > & out , int fact )
{
    out . clear ();
    for ( int k(0)
        ; k+1 < in . size ()
        ...
        )
    {
        ...
        {
            a = (float)f/fact;
            pt p ( in[k] . x_pos + a * ( in[k+1] . x_pos - in[k] . x_pos ) 
                 , in[k] . z_pos + a * ( in[k+1] . z_pos - in[k] . z_pos )
                 );
            std::cout << "k=" << k << " " << p.x_pos << " " << p.z_pos << std::endl;
            out.push_back(p);
        }
    }
}
...
int main( int argc , char ** argv )
{
    ...
    std::cout << "map migration" << std::endl;
    ...
    std::string vel_file_name( argv[arg] );

    arg++;

    std::string vel_s_file_name( argv[arg] );

    arg++;

    std::string epsilon_file_name( argv[arg] );

    arg++;

    std::string delta_file_name( argv[arg] );

    arg++;

    std::string nux_file_name( argv[arg] );

    arg++;

    std::string nuy_file_name( argv[arg] );

    arg++;

    std::string nuz_file_name( argv[arg] );



    arg++;

    std::string vel_2_file_name( argv[arg] );

    arg++;

    std::string vel_s_2_file_name( argv[arg] );

    arg++;

    std::string epsilon_2_file_name( argv[arg] );

    arg++;

    std::string delta_2_file_name( argv[arg] );

    arg++;

    std::string nux_2_file_name( argv[arg] );

    arg++;

    std::string nuy_2_file_name( argv[arg] );

    arg++;

    std::string nuz_2_file_name( argv[arg] );
    ...
    std::string horizon_input_file_name( argv[arg] );

    arg++;

    std::string horizon_output_file_name( argv[arg] );

    int wave_propagation = get_argument ( arg , argc , argv );
    ...
    SEPReader vel_reader       ( vel_file_name       . c_str () );
    SEPReader vel_s_reader     ( vel_s_file_name     . c_str () );
    SEPReader epsilon_reader   ( epsilon_file_name   . c_str () );
    SEPReader delta_reader     ( delta_file_name     . c_str () );
    SEPReader nux_reader       ( nux_file_name       . c_str () );
    SEPReader nuy_reader       ( nuy_file_name       . c_str () );
    SEPReader nuz_reader       ( nuz_file_name       . c_str () );

    SEPReader vel_2_reader     ( vel_2_file_name     . c_str () );
    SEPReader vel_s_2_reader   ( vel_s_2_file_name   . c_str () );
    SEPReader epsilon_2_reader ( epsilon_2_file_name . c_str () );
    SEPReader delta_2_reader   ( delta_2_file_name   . c_str () );
    SEPReader nux_2_reader     ( nux_2_file_name     . c_str () );
    SEPReader nuy_2_reader     ( nuy_2_file_name     . c_str () );
    SEPReader nuz_2_reader     ( nuz_2_file_name     . c_str () );

    int num_t = vel_reader . n1;
    int num_x = vel_reader . n2 
              * vel_reader . n3
              * vel_reader . n4
              * vel_reader . n5
              * vel_reader . n6
    ...
    memset ( &vel [0] , 0 , num_x2 * num_t2 );
    vel_reader . read_sepval ( & vel  [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * vs     = new float [ num_x2 * num_t2 ];
    memset ( &vs  [0] , 0 , num_x2 * num_t2 );
    vel_s_reader . read_sepval ( & vs   [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * delta  = new float [ num_x2 * num_t2 ];
    memset ( &delta[0] , 0 , num_x2 * num_t2 );
    delta_reader . read_sepval ( & delta [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * epsilon= new float [ num_x2 * num_t2 ];
    memset ( &epsilon[0] , 0 , num_x2 * num_t2 );
    epsilon_reader . read_sepval ( & epsilon [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * nux    = new float [ num_x2 * num_t2 ];
    memset ( &nux[0] , 0 , num_x2 * num_t2 );
    nux_reader . read_sepval ( & nux [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * nuy    = new float [ num_x2 * num_t2 ];
    memset ( &nuy[0] , 0 , num_x2 * num_t2 );
    nuy_reader . read_sepval ( & nuy [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * nuz    = new float [ num_x2 * num_t2 ];
    memset ( &nuz[0] , 0 , num_x2 * num_t2 );
    nuz_reader . read_sepval ( & nuz [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );



    float * vel2 = new float [ num_x2 * num_t2 ];
    memset ( &vel2 [0] , 0 , num_x2 * num_t2 );
    vel_2_reader . read_sepval ( & vel2  [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * vs2     = new float [ num_x2 * num_t2 ];
    memset ( &vs2  [0] , 0 , num_x2 * num_t2 );
    vel_s_2_reader . read_sepval ( & vs2   [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * delta2  = new float [ num_x2 * num_t2 ];
    memset ( &delta2[0] , 0 , num_x2 * num_t2 );
    delta_2_reader . read_sepval ( & delta2 [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * epsilon2= new float [ num_x2 * num_t2 ];
    memset ( &epsilon2[0] , 0 , num_x2 * num_t2 );
    epsilon_2_reader . read_sepval ( & epsilon2 [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * nux2    = new float [ num_x2 * num_t2 ];
    memset ( &nux2[0] , 0 , num_x2 * num_t2 );
    nux_2_reader . read_sepval ( & nux2 [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * nuy2    = new float [ num_x2 * num_t2 ];
    memset ( &nuy2[0] , 0 , num_x2 * num_t2 );
    nuy_2_reader . read_sepval ( & nuy2 [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );

    float * nuz2    = new float [ num_x2 * num_t2 ];
    memset ( &nuz2[0] , 0 , num_x2 * num_t2 );
    nuz_2_reader . read_sepval ( & nuz2 [ 0 ] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x2 * num_t2 );
    ...
    velocity_model < float > model2( num_x 
                                   , 1 
                                   , num_t 
                                   , scale_x
                                   , scale_y 
    ...
    std::vector < pt > horizon;

    read_horizon ( horizon_input_file_name , horizon );

    interpolate ( horizon , horizon , 100 );

    std::vector < pt > horizon_ini = horizon;
        ...
        )
    {
            ...
            )
        {
            ...
            {
                tmp_disp = (0.15*sqrtf(pow_2(x-horizon_ini[h].x_pos)+pow_2(t-horizon_ini[h].z_pos)));
                if ( tmp_disp < disp )
                {
                    disp = tmp_disp;
                }
            ...
            p2z[ k ] = 0.05*exp(-disp*disp/(128.0*width))*sin(2*M_PI*disp/(16.0*sqrt(width)));
            p2y[ k ] = 0;//*0.05*exp(-disp*disp/(128.0*width))*sin(2*M_PI*disp/(16.0*sqrt(width)));
            p2x[ k ] = 0;//((t>num_t*t_frac)?1:-1)*0.05*exp(-disp*disp/(128.0*width))*sin(2*M_PI*disp/(16.0*sqrt(width)));
            //p2x[ k ] = 0;//0.05*exp(-disp*disp/(128.0*width))*sin(2*M_PI*disp/(16.0*sqrt(width)));
            //p2y[ k ] = 0;//0.05*exp(-disp*disp/(128.0*width))*sin(2*M_PI*disp/(16.0*sqrt(width)));
            //p2z[ k ] = -0.05*exp(-disp*disp/(128.0*width))*sin(2*M_PI*disp/(16.0*sqrt(width)));
    ...
    float frac = 2; // MARK!
    float h = 0.00020 / frac;

    set_num_x ( num_x2 );
    set_num_z ( num_t2 );

    set_data_ptr ( data );
    set_scalar ( 0.4 / find_max ( data , num_t * num_x ) );
    ...
    for ( int x(0); x < num_x; ++x )
    {
        for ( int z(0); z < 2; ++z )
        {
            recorders . push_back ( recorder ( x , z ) );
        }
    }

    //for ( float x(-num_x2/2); x < num_x2/2 ; x += 1 )
    for ( int h ( 0 ) ; h+1 < horizon . size () ; ++ h )
    {
        ray r ( horizon [ h ] . x_pos
              , 0
              , horizon [ h ] . z_pos
              ,  (horizon [ h ] . z_pos - horizon [ h + 1 ] . z_pos)
              , 0 
              , -(horizon [ h ] . x_pos - horizon [ h + 1 ] . x_pos)
              );
        float n ( sqrtf ( r . vx * r . vx + r . vy * r . vy + r . vz * r . vz ) );
        r . vx /= n;
        r . vy /= n;
        r . vz /= n;
        r . th_init = 0;
        ...
        r . vy *= slow;
        r . vz *= slow;
        //std::cout << "h=" << h << " " << r.vx << " " << r.vz << " " << slow << std::endl;
        rays . push_back ( r );
    }

    for ( int k(0) ; k < frac*num_iter ; ++k )
    {
        if ( wave_propagation )
        {
        ...
        set_wave_scalar    ( 4 / find_max ( cx , num_x2 * num_t2 ) );
        set_reverse_scalar ( 4 / find_max ( cz , num_x2 * num_t2 ) );
        if ( k + 0 < frac*num_iter )
        {
        ...
        {
            int x_ind ( recorders[i] . x_pos );
            int z_ind ( recorders[i] . z_pos );
            recorders[i] . data_x . push_back ( p1x[z_ind+num_t*x_ind] );
            recorders[i] . data_y . push_back ( p1y[z_ind+num_t*x_ind] );
            recorders[i] . data_z . push_back ( p1z[z_ind+num_t*x_ind] );
        }
    }
    ...
    for ( int k(0) ; k < frac*num_iter ; ++k )
    {
        for ( int i(0) ; i < recorders . size () ; ++i )
        {
            int size ( recorders[i] . data_x . size () );
            int x_ind ( recorders[i] . x_pos );
            int z_ind ( recorders[i] . z_pos );
            p1x[z_ind+num_t*x_ind] = recorders[i] . data_x[size-1-k];
            p1y[z_ind+num_t*x_ind] = recorders[i] . data_y[size-1-k];
            p1z[z_ind+num_t*x_ind] = recorders[i] . data_z[size-1-k];
            if ( k > 0 )
            {
                p2x[z_ind+num_t*x_ind] = recorders[i] . data_x[size-1-k-1];
                p2y[z_ind+num_t*x_ind] = recorders[i] . data_y[size-1-k-1];
                p2z[z_ind+num_t*x_ind] = recorders[i] . data_z[size-1-k-1];
            }
        }
        if ( wave_propagation )
        propagate_anisotropic_2 ( h 
        ...
        if ( k + 0 < frac*num_iter )
        {
            for ( int i(0) ; i < rays . size () ; ++i )
            {
                             ...
                             );
                {
                    ...
                    int x_ind ( (int)(r.x/scale_x) );
                    if ( z_ind >= 0 && z_ind < num_t && x_ind >= 0 && x_ind < num_x )
                    data[ z_ind + num_t * x_ind ] = 100;
                }
            }
        }
    }

    std::vector < pt > out_horizon;
    std::cout << "output horizon" << std::endl;
    for ( int k(1) ; k+1 < rays . size () ; ++k )
    {
        //std::cout << k << " " << rays[k].x << " " << rays[k].z << std::endl;
        if ( sqrtf ( ( rays[k].x - rays[k-1].x ) * ( rays[k].x - rays[k-1].x ) + ( rays[k].z - rays[k-1].z ) * ( rays[k].z - rays[k-1].z ) ) < 50 
          && sqrtf ( ( rays[k].x - rays[k+1].x ) * ( rays[k].x - rays[k+1].x ) + ( rays[k].z - rays[k+1].z ) * ( rays[k].z - rays[k+1].z ) ) < 50 
           )
        out_horizon . push_back ( pt ( rays[k].x , rays[k].z ) );
    }

    write_horizon ( horizon_output_file_name , out_horizon );

    SEPWriter writer ( output_file_name . c_str () 
                     , vel_reader . o1 , vel_reader . d1 , num_t
                     , vel_reader . o2 , vel_reader . d2 , num_x
                     , vel_reader . o3 , vel_reader . d3 , 1
                     , vel_reader . get_header_labels ()
                     , vel_reader . get_sort_order ()
                     , (output_file_name + std::string("@")) . c_str()
                     );

    writer . OpenDataFile ( (output_file_name + std::string("@")) . c_str() );

    writer . write_sepval ( (float*)&data[0] , vel_reader . o1 , vel_reader . o2 , vel_reader . o3 , num_x * num_t );
