...
int main( int argc , char ** argv )
{
        ...
        )
    {
        ...
        {
            data[t+x*num_t2] = 3000.0;
            vel[k]     = data[t+x*num_t2];
            rho[k]     = 1/(vel[k]*vel[k]);
            delta[k]   = anis * vel[k] / 3000.0;
            epsilon[k] = anis * vel[k] / 3000.0;
        ...
        )
    {
            ...
            )
        {
            ...
            p2x[ k ] = 0;
            p2y[ k ] = 0.05*exp(-disp*disp/(128.0*width))*sin(2*M_PI*disp/(16.0*sqrt(width)));
            p2z[ k ] = 0;
    ...
    for ( float th(-M_PI); th <= M_PI; th += 0.005 )
    {
    ...
    float inv_max_w = 1 / find_max ( cy , num_x*num_t2 );
    float max_v = find_max ( data   , num_x*num_t  );

    for ( int i(0) ; i < rays . size () ; ++i )
    {
        ray & r ( rays [ i ] );
        data[ (int)(r.z/scale_z) + num_t * ( (int)(r.x/scale_x) ) ] = 0;
    }
        ...
        )
    {
        ...
        {
            data[k] -= 0.5 * cy[t+x*num_t2] * max_v * inv_max_w;
        }
    }
