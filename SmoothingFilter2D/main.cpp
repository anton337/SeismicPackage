...
int main( int argc , char ** argv )
{
    ...
    std::cout << "done get arg" << std::endl;
    ...
    float total_val = 0 , val , r ;
    for ( int fx(0) ; fx < width ; ++fx )
    {
        for ( int ft(0) ; ft < width ; ++ft )
        {
            r = sqrt ( fx*fx + ft*ft ) / width;
            val = exp ( - r * r );
            filt[ft+fx*num_t] = val;
            total_val += val;
            std::cout << val << " \t";
            if ( num_t - ft < num_t )
            {
                filt[num_t-ft+fx*num_t] = val;
                total_val += val;
            }
            if ( num_batch - fx < num_batch )
            {
                filt[ft+(num_batch-fx)*num_t] = val;
                total_val += val;
            }
            if ( num_t - ft < num_t && num_batch - fx < num_batch )
            {
                filt[num_t-ft+(num_batch-fx)*num_t] = val;
                total_val += val;
            }
        }
        std::cout << std::endl;
