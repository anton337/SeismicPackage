...
long total_ind_x = 4;//4;//12;
long total_ind_y = 4;//4;//12;
long total_ind_z = 1;//4;//12;

long distributed_index;

int port_duplicity = 0;//60;

std::vector<Server> server(port_duplicity);

float * output_canvas = new float [ (total_ind_x*62) * (total_ind_y*62) ];
...
void create_server ( Server * s , int distributed_index , int k )
{
    ...
    s -> Create ( network_list[distributed_index] . port+k , network_list . size () - 1 );
}
...
int main( int argc , char ** argv )
{
    ...
    distributed_tile[0] = total_ind_x;
    distributed_tile[1] = total_ind_y;
    ...
    for ( int it_k(0) ; it_k < 2000 ; ++it_k  )
    {
        ...
        //condition . wait (lock);
        {
            ...
            {
            set_scalar ( 0.4 / find_max ( output_canvas , 62*62*total_ind_x*total_ind_y ) );
            set_wave_scalar ( 0.4 / find_max ( output_canvas , 62*62*total_ind_x*total_ind_y ) );
            set_reverse_scalar ( 0.4 / find_max ( output_canvas , 62*62*total_ind_x*total_ind_y ) );
            }
        }
