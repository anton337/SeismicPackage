...
int main( int argc , char ** argv )
{
        ...
        )
    {
        ...
        for ( float th(-M_PI/2); th <= M_PI/2; th += 0.01 )
        {
            ray r ( scale_x*0 
            ...
            )
        {
            ...
            for ( int trace_iter(0) ; trace_iter < 1 ; ++trace_iter )
            {
                ...
                {   
                    ...
                    for ( int iter(0)
                        , spread
                        , size ( frac*num_iter*factor*scale_x )
                        ; iter < size
                        ; ++iter
                        )
                    {
                        spread = 1 + 10.0f * ( (float)iter / size );
                        solver . RK4 ( h/factor 
                                     , r . x 
                                     , r . vx
                                     , r . y 
                                     ...
                                     );
                        {
                            ...
                            for ( int ind_y(-spread)
                                ; ind_y <= spread
                                ; ++ind_y
                                )
                            {
                                for ( int ind_z(-spread)
                                    ; ind_z <= spread
                                    ; ++ind_z
                                    )
                                {
